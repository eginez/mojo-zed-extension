...
impl zed_extension_api::Extension for MojoExtension {
    ...
    fn language_server_command(
        ...
        let command = self.find_mojo_lsp_server(worktree)
            .ok_or_else(|| "mojo-lsp-server not found. Please install Mojo and ensure mojo-lsp-server is in your PATH. Visit https://docs.modular.com/mojo/ for installation instructions.".to_string())?;
...
mod tests {
    ...
    fn test_path_checking_logic() {
        ...
        let constructed_path = format!("{}/.pixi/envs/default/bin/mojo-lsp-server", temp_dir.to_string_lossy());
